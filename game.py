...
# after how many frames the player state will be updated 
...
class MOG:
  ...
  @staticmethod
  def is_tile(what):
    return what != None and (what.object_type == MOG.OBJECT_TILE or what.object_type == MOG.OBJECT_TRAMPOLINE)

  ## Makes an instance of MOG based on provided string.
  #
  #  @param object_string string representing the object
  #  @return MOG instance or None (if the string represented
  #          no object)
  ...
  def get_instance_from_string(object_string):
    ...
    if object_string[0] == "X":
      result.object_type = MOG.OBJECT_FINISH
    elif object_string[0] == "E":
      result.object_type = MOG.OBJECT_EGG
    elif object_string[0] == "C":
      result.object_type = MOG.OBJECT_COIN
    elif object_string[0] == "P":
      result.object_type = MOG.OBJECT_PLAYER
    elif object_string[0] == "T":
      result.object_type = MOG.OBJECT_TRAMPOLINE
    elif object_string[0] == "S":
      result.object_type = MOG.OBJECT_SPIKES
    elif object_string[0] == "F":
      result.object_type = MOG.OBJECT_ENEMY_FLYING
      result.enemy_id = int(object_string[2:])
    elif object_string[0] == "G":
      result.object_type = MOG.OBJECT_ENEMY_GROUND
      result.enemy_id = int(object_string[2:])
    else:    # tile
      result.object_type = MOG.OBJECT_TILE
  ...
  def __init_attributes(self):
    self.object_type = MOG.OBJECT_TILE
    self.tile_id = 0
    self.tile_variant = 1
    self.enemy_id = 0

  def __str__(self):
    if self.object_type == MOG.OBJECT_TILE:
      return "t"
    if self.object_type == MOG.OBJECT_COIN:
      return "C"
    if self.object_type == MOG.OBJECT_EGG:
      return "E"
    if self.object_type == MOG.OBJECT_PLAYER:
      return "P"
    if self.object_type == MOG.OBJECT_ENEMY_FLYING:
      return "F"
    if self.object_type == MOG.OBJECT_ENEMY_GROUND:
      return "G"
    if self.object_type == MOG.OBJECT_FINISH:
      return "F"
    return "?"
...
class Level:
  ...
  def load_from_file(self,filename):
    ...
    while line_number < len(content):
      if content[line_number] == "name:":
        ...
        line_number += 1
        self.outside_tile = MOG()
        self.outside_tile.object_type = MOG.OBJECT_TILE
        ...
        while True:              # load the map grid
          ...
          for pos_x in range(len(helper_list)):
            ...
            helper_object = MOG.get_instance_from_string(helper_list[pos_x])

            if helper_object == None:
              self.map_array[pos_x][pos_y] = helper_object
            elif helper_object.object_type == MOG.OBJECT_PLAYER:
              self.player = Player(self)
              self.player.position_x = pos_x + 0.5
              self.player.position_y = pos_y + 0.5
            elif helper_object.object_type == MOG.OBJECT_ENEMY_FLYING:
              self.enemies.append(Enemy(self,Enemy.ENEMY_FLYING))
              self.enemies[-1].position_x = pos_x + 0.5
              self.enemies[-1].position_y = pos_y + 0.5
            elif helper_object.object_type == MOG.OBJECT_ENEMY_GROUND:
              self.enemies.append(Enemy(self,Enemy.ENEMY_GROUND))
              self.enemies[-1].position_x = pos_x + 0.5
              self.enemies[-1].position_y = pos_y + 0.5
            else:
              if helper_object.object_type == MOG.OBJECT_EGG:
                self.eggs_left += 1
              elif helper_object.object_type == MOG.OBJECT_COIN:
                self.coins_total += 1
  ...
  def update(self):
    ...
    if object_at_player_tile != None:
      if object_at_player_tile.object_type == MOG.OBJECT_COIN:
        self.sound_player.play_coin()
        self.map_array[player_tile_x][player_tile_y] = None
        self.coins_collected += 1
      elif object_at_player_tile.object_type == MOG.OBJECT_EGG:
        self.sound_player.play_click()
        self.map_array[player_tile_x][player_tile_y] = None
        self.eggs_left -= 1
      elif object_at_player_tile.object_type == MOG.OBJECT_FINISH:
        if self.eggs_left <= 0:
      ...
      elif object_at_player_tile.object_type == MOG.OBJECT_SPIKES:
        self.set_lost()
        return

    if object_under_player_tile != None and object_under_player_tile.object_type == MOG.OBJECT_TRAMPOLINE and not self.player.air():
  ...
  def __init_attributes(self):
    ...
    #  be None (representing nothing) or a MOG
    self.map_array = None
    ## contains a MOG representing a tile with which the area
  ...
  ## Gets the MOG at given position in the map with map
  #  boundary check.
  #
  #  @param x x position
  #  @param y y position
  #  @return MOG at given position (can be also None), if the
  #          position provided is outside the map area, the
  #          MOG representing the outside tile is returned
...
class Moo(object):
  ...
  #
  #  @param with_what object to check the collision with (Moo)
  ...
  def air(self):
    ...
    if MOG.is_tile(self.level.get_at(int(self.position_x),tile_y)):
      ground = tile_y - lower_border
    ...
    if dx > 0:
      ...
      for i in range(occupied_cells[1],occupied_cells[3] + 1):
        ...
        for j in range(occupied_cells[2] + 1,occupied_cells[2] + 3):  # checks the following two cells
          if MOG.is_tile(self.level.get_at(j,i)):
            value = j
            break
    ...
    if dy > 0:
      ...
      for i in range(occupied_cells[0],occupied_cells[2] + 1):
        ...
        for j in range(occupied_cells[3] + 1,occupied_cells[3] + 3):  # checks the following two cells
          if MOG.is_tile(self.level.get_at(i,j)):
            value = j
            break
...
class Renderer:
  ...
  def __init_attributes(self):
    ...
    #  from 1, index 0 contains a Tile
    self.tile_images = {}
  ...
  def set_level(self, level):
    ...
    for tile in level.tiles:
      ...
      self.tile_images[tile[0]].append(Tile())
  ...
  def __get_top_layer(self, x, y):
    center = not MOG.is_tile(self._level.get_at(x, y - 1))
    left = not MOG.is_tile(self._level.get_at(x, y - 1)) and not MOG.is_tile(self._level.get_at(x - 1, y)) and not MOG.is_tile(self._level.get_at(x - 1, y - 1))
    right = not MOG.is_tile(self._level.get_at(x, y - 1)) and not MOG.is_tile(self._level.get_at(x + 1, y)) and not MOG.is_tile(self._level.get_at(x + 1, y - 1))
    ...
    for j in range(self.visible_tile_area[1],self.visible_tile_area[3]):  # render only visible area
      for i in range(self.visible_tile_area[0],self.visible_tile_area[2]):
        ...
        if map_grid_object == None:
          ...
          if map_grid_object.object_type == MOG.OBJECT_TILE:
          ...
          elif map_grid_object.object_type == MOG.OBJECT_SPIKES:
            result.blit(self.spikes_image,(x,y))
          elif map_grid_object.object_type == MOG.OBJECT_EGG:
            result.blit(self.egg_image,(x + 50,y + 50))
          elif map_grid_object.object_type == MOG.OBJECT_COIN:
            result.blit(self.coin_images[animation_frame % len(self.coin_images)],(x + 20,y))
          elif map_grid_object.object_type == MOG.OBJECT_TRAMPOLINE:
            result.blit(self.trampoline_image,(x,y))
          elif map_grid_object.object_type == MOG.OBJECT_FINISH:
...
## A decorator that moves given Moo object acoording to forces it
#  computes.

class FC:
  def __init_attributes(self):
    ## reference to decorated object (Moo)
...
class Game:
  ...
  def __init__(self, name, fullscreen, sound):
    ...
    pygame.display.set_caption("Quacko")
    ...
    while not done:
      ...
      if self.state == Game.STATE_IN_GAME:
        ...
        if self.level.state == Level.STATE_PLAYING:
          if self.key_up:
            if not self.level.player.state in [Player.PLAYER_STATE_JUMPING_UP, Player.PLAYER_STATE_JUMPING_DOWN] and not self.level.player.air():
